...
class Constants {
    ...
    static let invalidPhone = "Invaild phone number"
    ...
    static let phoneFormat = "SELF MATCHES[c] %@"
    static let passwordRegEx = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[d$@$!%*?&#])[A-Za-z\\dd$@$!%*?&#]{8,}"
    static let phoneRegEx = "^(01)[0-2,5]{1}[0-9]{8}"
    ...
    static let addressMsg = "Your address is already exists or there is something wrong with city or country, so please check your data again"
